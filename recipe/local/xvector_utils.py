...
class XvectorClassifier():
    ...
    def load(klass, model_path):
        with open(model_path, 'rb') as model_pickle:
            model, class_names = joblib.load(model_pickle)
    ...
    def save(self, model_path):
        with open(model_path, 'wb') as model_file:
            joblib.dump((self._model, self._class_names), model_file)
...
def load_xvectors_and_labels(logger, labels_stm, xvectors_ark):
    ...
    #labels = np.asarray(labels, dtype=np.int)  #  py35
    labels = np.asarray(labels, dtype=int)  #  now
...
def make_xvectors(logger, wav_scp, labels_stm, output_ark, num_cpus=-1):
    ...
    logger.info("Running 'cd {} && {}'".format(recipe_dir, cmd))
...
def main():
    ...
    if arguments['visualize']:
        ...
        with open(dataset_path, 'wb') as dataset_file:
            joblib.dump((dataset, metadata), dataset_file)
